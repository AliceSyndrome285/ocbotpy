...
* **智能对话:** 通过 OpenConversationWorkshop API 实现智能对话，根据用户的问题提供相应的答案。
* **记忆和总结:**  将对话历史记录发送到一个独立的记忆处理应用，并定期对对话内容进行总结，发送给 OpenConversationWorkshop API，帮助维持上下文和提升对话连贯性。
...
* **BotPY 客户端:** 负责接收群消息、@机器人消息，并调用 OpenConversationWorkshop API 获取回复。
